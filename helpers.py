...
try:
    ...
    from helpers import DataHelpers, ModelHelpers
    BACKEND_READY = True
...
@app.get("/api/model-parameters/{model_type}")
async def get_model_parameters(model_type: str):
    try:
        parameters = ModelHelpers.get_model_parameters(model_type)  # Appel de la méthode statique
        ...
        logger.error(f"❌ Erreur paramètres modèle {model_type}: {str(e)}")
        raise HTTPException(status_code=400, detail=f"Erreur lors de la récupération des paramètres: {str(e)}")
